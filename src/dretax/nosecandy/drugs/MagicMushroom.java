...
public class MagicMushroom extends BaseDrug
{
	Addict a;

	public MagicMushroom(NoseCandy plugin)
	{
		super(plugin, Config.MagicMushroomName, Config.MagicMushroomTex);
	}

	public boolean onItemInteract(SpoutPlayer player, SpoutBlock block, BlockFace face)
	{
		CraftPlayer p = (CraftPlayer)player;
		p.getHandle().addEffect(new MobEffect(8, 800, 3));
		p.getHandle().addEffect(new MobEffect(9, 300, 1));
		p.getHandle().addEffect(new MobEffect(15, 200, 3));
		p.getHandle().addEffect(new MobEffect(4, 1000, 3));
